...
export const sendExpiredStart = () => async (dispatch: AppDispatch, getState: AppGetState) => {
  const text = 'Началась задержка загрузки'
  dispatch(sendToTg(text))
}
export const sendExpiredEnd = () => async (dispatch: AppDispatch, getState: AppGetState) => {
  const text = 'Задержка загрузки закончилась'
  dispatch(sendToTg(text))
}

export const sendDownloadingReport = () => async (dispatch: AppDispatch, getState: AppGetState) => {
...
export const sendToTg = (text: string) => async (dispatch: AppDispatch, getState: AppGetState) => {
  const state = getState();
  const app = state.app;
