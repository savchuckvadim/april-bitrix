...
import { applyCommercialEdit, applyTaxChange } from '../price';
import { buildTotalRow, createSet, upsertRow } from '../set';
...
describe('паритет цен: универсальная линейка', () => {
    it('Классик = abs × region.abs × coefficient', () => {
        ...
        // 2 × 3975.6 × 1.25 = 9939
        expect(row!.price.base).toBe(9939);
    });
});
...
describe('паритет цен: сервисы', () => {
    it('консалтинг: abs × region.abs (без коэффициента), × срок', () => {
        ...
        // 1 × 3975.6 × 12 = 47707.2, скидка 0.8
        expect(row!.price.base).toBe(47707.2);
        expect(row!.price.current).toBe(38165.76);
    });
