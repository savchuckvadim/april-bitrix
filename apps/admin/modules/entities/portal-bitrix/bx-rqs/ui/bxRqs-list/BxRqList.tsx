...
import { BxRqTable } from '../bxRqs-table';
import { Button } from '@workspace/ui/components/button';
import { ConfirmDialog } from '@/modules/shared/ui/confirm-dialog';
import { BxRqResponseDto } from '@workspace/nest-api';
import { getUrlToBxRq, getUrlToEditBxRq } from '../../lib/utils/get-url-util';

export function BxRqList({ portalId }: { portalId: number }) {
    const router = useRouter();
    const { data: bxRqses, isLoading } = useBxRqs({portal_id: portalId.toString() });
    ...
    const handleRowClick = (bxRqs: BxRqResponseDto) => {
        router.push(getUrlToBxRq(portalId, bxRqs.id));
    };

    const handleEdit = (bxRqs: BxRqResponseDto) => {
        router.push(getUrlToEditBxRq(portalId, bxRqs.id));
    };
