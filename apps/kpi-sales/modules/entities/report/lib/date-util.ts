...
export const getDatesByMode = (mode: EReportDateMode): {
    from: string,
    to: string
} => {
...
export const getDatesByRememberFilter = (filter: IFilterResponse): {
    from: string,
    to: string,
    isMode: boolean
} => {
    ...
    const isMode = targetModes.includes(comeFrom as EReportDateMode) ||
...
export const modifyDateToReportRequest = (from: string, to: string): {
    from: string,
    to: string
} => {
