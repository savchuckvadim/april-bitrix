...
const infoblockSlice: Slice<DInfoblockState> = createSlice({
  ...
  reducers: {
    ...
    addInfoblockToGroup(
      state,
      action: PayloadAction<{ groupId: number; infoblock: SIBlock }>,
    ) {
  ...
  extraReducers: (builder) => {
    builder
      .addMatcher(infoblockAPI.endpoints.fetchIBlocks.matchPending, (state) => {
        state.isLoading = true;
      })
      .addMatcher(
        ...
        (state, { payload }) => {
          ...
          state.groups = state.groups.map((group) => {
            return {
              ...group,
              infoblocks: payload
                .filter((iblock) => iblock.groupId === group.id)
                .map((iblock) => ({
                  ...iblock,
                  checked: false,
                })),
...
// Экспорт actions
export const infoblockActions: typeof infoblockSlice.actions =
  infoblockSlice.actions;
