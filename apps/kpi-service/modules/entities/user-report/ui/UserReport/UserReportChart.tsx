...
export const UserReportChart: React.FC<UserReportChartProps> = ({
    ...
    const availableScales = useMemo(() => {
        ...
        const dates = filteredData
            .map(item => {
                const dateValue = item.service_ork_history_ork_event_date?.value?.value;
        ...
        console.log('Chart debug:', {
            ...
            sampleDates: filteredData.slice(0, 3).map(item => ({
                original: item.service_ork_history_ork_event_date?.value?.value,
                parsed: parseCustomDate(item.service_ork_history_ork_event_date?.value?.value || '')?.toISOString()
            }))
        });
    ...
    const eventTypes = useMemo((): EventTypeData[] => {
        ...
        filteredData.forEach(item => {
            const action = item.service_ork_history_ork_event_action?.value?.name || 'Неизвестно';
            const type = item.service_ork_history_ork_event_type?.value?.name || 'Неизвестно';
