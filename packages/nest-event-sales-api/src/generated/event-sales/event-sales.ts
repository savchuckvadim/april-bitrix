...
export const getEventSales = () => {
    ...
    /**
     * Перезаписывает ответы анкеты в поля клиента: лид и сделки получают весь состав (девять детальных «5К» + шесть «Разговора» + оба сводных), компания — только сводные («Хвост», «Пять К»). Жёсткий серверный whitelist: ключи fiveK вне списка op_5k_* молча отбрасываются. Только перезапись (append нет) — повтор того же payload даёт тот же результат; повтор operationId в течение 24ч не пишется второй раз. Пустые values — no-op без похода в Битрикс. Неустановленное на портале поле пропускается с warning в ответе.
