...
export const canDropBlock = (
    targetPage: IOfferTemplatePage,
    block: IOfferTemplateBlock,
) => {
    let targetPageHeight =
        targetPage?.blocks.reduce((acc, block) => acc + block.height, 0) || 0;
...
export const getNewPage = (block: IOfferTemplateBlock, order: number) => {
    return {
        id: uuidv4(),
        order: order,
        type: 'letter',
