 ...
 * добавляется одной строкой здесь и одной в use-entity-warning-handlers.
 */
export type EntityWarningActionId =
    | 'fill-inn'
    | 'fill-company-title'
...
/**
 * Предупреждения общей шапки: чего не хватает, чтобы работать с клиентом
 * дальше. Все они про КЛИЕНТА, а не про конкретное дело — поэтому живут в
 * общей шапке над роут-слотом и видны на каждом экране, сразу под названиями.
 *
 ...
 */
export const useEntityWarnings = (): EntityWarning[] => {
    ...
    // Про прогноз предупреждения нет намеренно: шкала стоит тут же, в шапке,
    // и сама показывает и «не задан», и ошибку после неудачной отправки.
