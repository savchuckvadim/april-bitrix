...
export const useDepartment = () => {
    ...
    const handleSetCurrentDepartmentItem = useCallback((userId: number) => {
        ...
        const searchingUserInCurrent = current.find((user: BXUser) => user.ID === userId);
        let updatedCurrent = [...current];

        if (searchingUserInCurrent) {
            updatedCurrent = updatedCurrent.filter((user: BXUser) => user.ID !== userId);
        } else {
            const addingUser = items.find((user: BXUser) => user.ID === userId);
            if (addingUser) {
    ...
    const handleSetCurrentGroup = useCallback((groupId: number) => {
        ...
        if (searchingGroup) {
            if (isInCurrent) {
                ...
                searchingGroup.USERS?.forEach((user: BXUser) => {
                    currentUsers = currentUsers.filter((currentUser: BXUser) => currentUser.ID !== user.ID);
                });
            } else {
                currentGroups.push(searchingGroup);
                searchingGroup.USERS?.forEach((user: BXUser) => {
                    if (!currentUsers.find((currentUser: BXUser) => currentUser.ID === user.ID)) {
