...
export class BitrixBaseApi {
    ...
    /**
     * Подгонка фрейма под контент с нижней границей.
     *
     * Отличие от `getFit()` в одном: у фрейма появляется минимальный размер.
     * Это нужно на маленьких мониторах, где встройка-вкладка выдаёт узкий
     * блок и приложение оказывается в щели.
     *
     * Размеры НЕ считаем от `window.screen`: экран — это физический монитор, он
     * ничего не знает ни про масштабирование системы, ни про то, что окно
     * браузера занимает половину рабочего стола. Ровно на этом ползла ширина в
     * старом конструкторе (`resizeWindow(screen.width * 0.73, …)`). Высоту и
     * ширину считает сам SDK по узлу контента, а мы задаём только пол.
     */
    ...
    public scrollParentTo(scroll = 0) {
        if (this.inFrame) {
            return this.bx.parent.scrollParentWindow(scroll);
        }
        return null;
    }
