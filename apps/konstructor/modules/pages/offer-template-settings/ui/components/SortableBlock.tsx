...
// import { isChanging } from "../../lib/choose-lib/render-lib/render.util";
...
//     if ('top' in block && typeof block.top === 'number') positionClasses.push(`top-${block.top}`);
//     if ('left' in block && typeof block.left === 'number') positionClasses.push(`left-${block.left}`);
//     if ('right' in block && typeof block.right === 'number') positionClasses.push(`right-${block.right}`);
//     if ('bottom' in block && typeof block.bottom === 'number') positionClasses.push(`bottom-${block.bottom}`);
...
//   const { current } = useOfferTemplateSettings()
//   const {
//     isOver,
//     attributes,
//     listeners,
...
//         {isChanging(block) && <Button size={'sm'} onClick={() => setEditeble(block)} className=" btn btn-xs bg-gray-100 "><Edit color='gray' /></Button>}
