...
export const UserReportFilters: React.FC<UserReportFiltersProps> = ({
    ...
    const filterOptions = useMemo(() => {
        ...
        report.forEach(item => {
            // Action filter
            const actionCode = item.service_ork_history_ork_event_action?.value?.code;
            const actionName = item.service_ork_history_ork_event_action?.value?.name;
            if (actionCode && actionName) {
                actionMap.set(actionCode, (actionMap.get(actionCode) || 0) + 1);
            }

            // Type filter
            const typeCode = item.service_ork_history_ork_event_type?.value?.code;
            const typeName = item.service_ork_history_ork_event_type?.value?.name;
            if (typeCode && typeName) {
                typeMap.set(typeCode, (typeMap.get(typeCode) || 0) + 1);
            }

            // Initiative filter
            const initiativeCode = item.service_ork_history_ork_event_initiative?.value?.code;
            const initiativeName = item.service_ork_history_ork_event_initiative?.value?.name;
        ...
        return {
            actions: Array.from(actionMap.entries()).map(([code, count]) => ({
                code,
                name: report.find(item => item.service_ork_history_ork_event_action?.value?.code === code)?.service_ork_history_ork_event_action?.value?.name || code,
                count
            })),
            types: Array.from(typeMap.entries()).map(([code, count]) => ({
                code,
                name: report.find(item => item.service_ork_history_ork_event_type?.value?.code === code)?.service_ork_history_ork_event_type?.value?.name || code,
                count
            })),
