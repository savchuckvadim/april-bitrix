...
const initialState = {
    [GLOBAL_STATE_PROP.SUPPLY]: {
        name: 'Тип Доступа',
        values: [{ id: 0, title: GLOBAL_SUPPLY_NAME.INTERNET }, { id: 1, title: GLOBAL_SUPPLY_NAME.PROXIMA }]
    } as GlobalStateItemSupply,

    [GLOBAL_STATE_PROP.COMPLECT]: {
        name: 'Тип Комплекта',
        values: [{ id: 0, title: GLOBAL_COMPLAECT_TYPE_NAME.UNIVERSAL }, { id: 1, title: GLOBAL_COMPLAECT_TYPE_NAME.PROF }]
    ...
    current: {
        [GLOBAL_STATE_PROP.SUPPLY]: { id: 0, title: GLOBAL_SUPPLY_NAME.INTERNET } as GlobalSupplySelectItem,
        [GLOBAL_STATE_PROP.COMPLECT]: { id: 1, title: GLOBAL_COMPLAECT_TYPE_NAME.PROF } as GlobalComplectSelectItem,
...
const documentComplectSettingsSlice = createSlice({
    ...
    reducers: {
        setFetchedRegions: (
            ...
            const resultRegions = payload.regions.map(region => ({ ...region, id: region.number }))
        ...
        setCurrent: (
            ...
            const payload = action.payload;
            const currentItem = state[payload.type].values.find(value => value.id === payload.valueId)
            if (currentItem) {
                // @ts-ignore
...
export const documentComplectSettingsReducer = documentComplectSettingsSlice.reducer;

// Экспорт actions
export const documentComplectSettingsActions = documentComplectSettingsSlice.actions;