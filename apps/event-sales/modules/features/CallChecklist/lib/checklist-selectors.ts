...
export const isChecklistFieldMissing = (
    ...
    /*
     * Каналы `dto` и `smart`: закрыть вопрос может ТОЛЬКО ответ.
     *
     * Их ответ уезжает payload'ом отправки, а не живёт в карточке, — и
     * значение, стоящее в CRM, ответом не становится само по себе. Раньше
     * становилось: сумма продажи стояла в сделке, вопрос выглядел закрытым,
     * ответа не возникало, `sale.opportunity` уходила пустой, и сервер-гард
     * отвергал весь отчёт о продаже (400 «заполните сумму сделки и дату
     * первой оплаты» при заполненной сделке).
     *
     * Трения это не добавляет: значение карточки засевается ОТВЕТОМ при
     * появлении вопроса на экране (`seedChecklistDtoAnswers`) — менеджер
     * видит то же, что видел, но теперь оно действительно уезжает.
     *
     * Канал `text` намеренно живёт по прежнему правилу: его ответ — строка
     * комментария, и портальные текстовые вопросы, закрытые значением поля,
     * запирать отправку задним числом нельзя.
     */
    if (resolved.def.channel === 'dto' || resolved.def.channel === 'smart') {
        return !savedValue;
    }
