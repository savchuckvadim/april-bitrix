...
interface ProcessShellProps {
    ...
    title: string;
    /**
     * Раздел, который показывает рама: базовый путь, вкладки и подпись группы
     * в боковом меню. Не задан — раздел продаж (обратная совместимость).
     */
 ...
 *
 * Рама не знает, какой раздел показывает: вкладки, базовый путь и подпись
 * группы приходят определением раздела и раздаются вниз через контекст.
 */
export const ProcessShell: FC<ProcessShellProps> = ({
    eyebrow,
    title,
    definition = SALES_SECTION,
    scroll = 'island',
    children,
}) => {
    ...
    return (
        <SectionProvider definition={definition}>
            {/*
            ...
            >
                ...
                <div className="flex min-h-0 w-full flex-1">
                    ...
                    >
                         ...
                         */}
                        {/* pr-3, а не общий px-1: справа стоит переключатель
                            оформления, и при четырёх пикселях кнопка цвета
                            липла к самому краю окна. */}
                        <div className="flex h-14 shrink-0 items-center gap-2 px-1 pr-3">
                            {!isOpen && (
                                <Button
                                    variant="ghost"
                                    size="icon"
                            ...
                            <Link
                                href="/"
                                className="ml-2 shrink-0 font-bold lg:hidden"
                            >
                            ...
                            <div className="text-muted-foreground ml-auto hidden items-center gap-3 text-xs lg:flex">
                                <ZoomControls
                                    ...
                                    onReset={zoom.reset}
                                />
                                {/* Переключатель прижат к правому краю строки,
                                    поэтому палитра обязана расти влево. */}
