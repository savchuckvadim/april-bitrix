...
/**
 * Кап полноэкранного Processing: дольше держать пользователя «в заложниках»
 * нельзя — после капа рендерим каркас страницы (фильтры доступны, можно
 * изменить период/состав и переотправить запрос), а загрузку показывает
 * сам отчётный блок компактным лоадером.
 */
const PROCESSING_CAP_MS = 50_000;
