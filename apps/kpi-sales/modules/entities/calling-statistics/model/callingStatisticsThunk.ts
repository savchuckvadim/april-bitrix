...
export const getCallingStatistics = (reportData: ReportRequest) =>
    async (dispatch: AppDispatch, getState: AppGetState) => {
        const isLoading = getState().callingStatistics.isLoading;
        if (!isLoading) {
            dispatch(callingStatisticsActions.setLoading(true));

            const reportResponse: ReportCallingData[] | null = await fetchCallingStatistics(reportData);
            // try {
            //     const data = {
            ...
            // }   
            dispatch(callingStatisticsActions.setFetched(
                reportResponse
            ));
