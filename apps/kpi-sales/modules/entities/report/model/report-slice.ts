...
const reportSlice = createSlice({
    ...
    initialState,
    reducers: {
        setFetchedReport: (state: ReportState, action: PayloadAction<{
        ...
        setUserReport: (state: ReportState, action: PayloadAction<ReportData>) => {
            const isUpdated = state.report.some(report => report.user.ID === action.payload.user.ID);
            if (isUpdated) {
                state.report = state.report.map(report =>
                    report.user.ID === action.payload.user.ID ? action.payload : report
                );
            } else {
                state.report.push(action.payload);
            }
        },
        setFetchedFilter: (state: ReportState, action: PayloadAction<{
            filter: Array<Filter>;
            currentFilter: Array<FilterInnerCode> | null;
        }>) => {
            state.filter = action.payload.currentFilter && action.payload.currentFilter.length
                ? action.payload.currentFilter
                : action.payload.filter.map(f => f.innerCode);
        },
        setFilterLoadingStatus: (state: ReportState, action: PayloadAction<boolean>) => {
            state.isFilterLoading = action.payload;
        },
        setCurrentFilter: (state: ReportState, action: PayloadAction<FilterInnerCode>) => {
            const isNeedCut = state.filter.includes(action.payload);
            state.filter = isNeedCut
                ? state.filter.filter(f => f !== action.payload)
                : [...state.filter, action.payload];
        },
        setFetchedActions: (state: ReportState, action: PayloadAction<{
            actions: Array<Filter>;
            currentFilter: Array<FilterInnerCode> | null;
        }>) => {
            const current = action.payload.currentFilter && action.payload.currentFilter.length
                ? action.payload.actions.filter(act => action.payload.currentFilter?.includes(act.innerCode))
        ...
        // },
        setChangedDate: (state: ReportState, action: PayloadAction<{
        ...
        setLoadingReportStatus: (state: ReportState, action: PayloadAction<boolean>) => {
        ...
        setCurrentGroup: (state: ReportState, action: PayloadAction<number>) => {
