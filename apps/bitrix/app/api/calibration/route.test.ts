/**
 * Маршрут приёма онлайн-брифа: бэк подменён, проверяем ровно обязанности
 * двери — что уходит в Telegram и с какой темой, части длинного брифа,
 * honeypot, повтор, кривое тело и сбой бэка.
 */

import { afterEach, describe, expect, it, vi } from 'vitest';
import type { TelegramSendMessageDto } from '@workspace/nest-api';
import { TELEGRAM_PART_MAX_LENGTH } from './lib/split-telegram-text';
...
describe('штатная отправка', () => {
    it('короткий бриф уходит одним сообщением с темой «Бриф калибровки: компания / домен»', async () => {
        const response = await POST(post(submission('1. Вопрос: ответ')));
        expect(response.status).toBe(200);
        expect(await response.json()).toEqual({ ok: true, parts: 1 });
        expect(sent).toHaveLength(1);
        expect(sent[0]?.text.startsWith('Бриф калибровки: ООО Ромашка / romashka.bitrix24.ru\n\n')).toBe(true);
        ...
        expect(sent[0]?.app).toBe('bitrix-site-calibration');
    });
    ...
    it('без реквизитов тема честно говорит, что их нет', async () => {
        ...
        expect(sent[0]?.text.startsWith('Бриф калибровки: реквизиты не указаны')).toBe(true);
...
describe('защита двери', () => {
    it('honeypot заполнен — «ок» без отправки', async () => {
        const response = await POST(post(submission('бот', { website: 'http://spam' })));
        expect(response.status).toBe(200);
    ...
    it('повтор того же протокола — 409 и второй раз не шлём', async () => {
        const body = submission('повторный бриф');
    ...
    it('сбой бэка — 502 с ошибкой, чтобы клиент предложил повторить', async () => {
        ...
        failNext.value = true;
        const response = await POST(post(submission('упадёт')));
        expect(response.status).toBe(502);
        expect((await response.json()).ok).toBe(false);
        // повтор после сбоя не считается дублем
        failNext.value = false;
        expect((await POST(post(submission('упадёт')))).status).toBe(200);
    });
});
