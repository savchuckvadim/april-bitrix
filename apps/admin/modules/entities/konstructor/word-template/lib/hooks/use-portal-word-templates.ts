...
export const usePortalWordTemplates = (portalId: string) => {
    return useQuery({
        queryKey: [WORD_TEMPLATE_LIST_CASH_KEY, 'portal', portalId],
        queryFn: async () => {
            const response = await helper.findByPortal(portalId);
            return response;
        },
        enabled: !!portalId,
    });
