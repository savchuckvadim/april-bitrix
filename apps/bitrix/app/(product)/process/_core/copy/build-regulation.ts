 ...
 *
 * Это главный артефакт страницы: заказчик крутит ползунки, отвечает на вопросы
...
export const buildRegulation = ({
    ...
    // Про потолок пишем, только когда он реально сработал: если ползунок лида
    ...
    if (model.leadCapStageId && isCapBinding) {
        ...
        lines.push(
            '',
            `Регламент: дальше стадии «${capped?.label}» работа в лиде не ведётся, даже если выкрутить ползунок лида на максимум.`,
        );
    }
    ...
    lines.push(
        '',
        RULE,
        'Пометка «в разработке» относится к этому документу: такой кусок здесь',
        'ещё дописывается. Всё остальное в нём доведено и согласовано.',
        '',
