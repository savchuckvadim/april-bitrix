...
import { EReportDateMode, ReportDateType } from '../model/types/report/report-type';
...
const DatesFilter: React.FC = () => {
    ...
    const [showDateRange, setShowDateRange] = React.useState(date.mode === EReportDateMode.RANGE);
    ...
    return (<div className='mb-5'>  <Label className='mb-2'>Период</Label>
        <div className="min-h-[60px] mt-5 flex flex-col xl:flex-row gap-2">

            <RadioGroup defaultValue={date.mode} className={`order-2 xl:order-1 flex flex-col flex-wrap  gap-2`}>
                ...
                <div className="flex items-center space-x-2">
                    <RadioGroupItem className='cursor-pointer' value={EReportDateMode.TODAY} id={EReportDateMode.TODAY} onClick={() => handleQuickSelect(EReportDateMode.TODAY)} />
                    <Label htmlFor={EReportDateMode.TODAY}>Сегодня</Label>
                </div>
                <div className="flex items-center space-x-2">
                    <RadioGroupItem className='cursor-pointer'
                        value={EReportDateMode.WEEK} id={EReportDateMode.WEEK} onClick={() => handleQuickSelect(EReportDateMode.WEEK)} />
                    <Label htmlFor={EReportDateMode.WEEK}>Hеделя</Label>
                </div>
                <div className="flex items-center space-x-2">
                    <RadioGroupItem className='cursor-pointer'
                        value={EReportDateMode.MONTH} id={EReportDateMode.MONTH} onClick={() => handleQuickSelect(EReportDateMode.MONTH)} />
