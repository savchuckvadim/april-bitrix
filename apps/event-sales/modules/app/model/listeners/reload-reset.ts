 ...
 *
 * Портальный КАТАЛОГ АНКЕТ (questionnaireCatalog) сюда НЕ входит, а
 * ОТВЕТЫ на анкеты (callChecklist) входят: состав вопросов портала
 * перезагрузкой карточки не меняется, и сброс каталога означал бы лишний
 * запрос состава на каждое ⟳ плюс окно, в котором вопросы исчезли с
 * экрана. Перечитывание состава решает сам каталог — по смене домена и
 * расхождению хэша (ensureQuestionnaireCatalog).
 *
